...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        infer_tp = self.config.rollout.tensor_model_parallel_size * self.config.rollout.data_parallel_size  # rollout.data_parallel_size一般为1，这个应该是数据太长，在tp的基础上对数据进一步划分
    ...
    def init_model(self):
        ...
        if self._is_actor or self._is_rollout:
            ...
            (
                ...
                use_remove_padding=use_remove_padding,
                use_fused_kernels=use_fused_kernels,
                enable_gradient_checkpointing=self.config.model.get("enable_gradient_checkpointing", False),  # 一般为True
                trust_remote_code=self.config.model.get("trust_remote_code", False),
                use_liger=self.config.model.get("use_liger", False),
                role="actor",
                enable_activation_offload=self.config.model.get("enable_activation_offload", False),  # 一般为False
            )
