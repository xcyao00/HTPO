...
def _compute_entropy_split_masks(
    entropy: torch.Tensor,
    response_mask: torch.Tensor,
    high_entropy_ratio: float = 0.2,
    clip_low_entropy_ratio: Optional[float] = None,
    clip_high_entropy_ratio: Optional[float] = None,
) -> tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor], Optional[torch.Tensor]]:
    """Split token-level entropy into low/high masks per sequence, with optional clip masks.

    For each sequence, the top ``high_entropy_ratio`` fraction of entropy values
    (i.e. the *highest* entropy tokens among valid tokens) forms the **high-entropy** set;
    the rest form the **low-entropy** set.

    Args:
        entropy: shape (bs, response_length)
        response_mask: shape (bs, response_length)
        high_entropy_ratio: fraction of tokens treated as the boundary
            (default 0.2 → top-20% entropy tokens are "high", bottom-80% are "low").
        clip_low_entropy_ratio: if given, also return a mask for the lowest
            ``clip_low_entropy_ratio`` fraction of the low-entropy tokens (used in group 1).
        clip_high_entropy_ratio: if given, also return a mask for the highest
            ``clip_high_entropy_ratio`` fraction of the high-entropy tokens (used in group 6).

    Returns:
        low_entropy_mask:   (bs, response_length)
        high_entropy_mask:  (bs, response_length)
        clip_low_entropy_mask:      (bs, response_length) or None
        clip_high_entropy_mask:     (bs, response_length) or None
    """
    low_masks, high_masks, clip_low_masks, clip_high_masks = [], [], [], []
    for i in range(len(entropy)):
        entropyi, response_maski = entropy[i], response_mask[i]
        valid_entropy = entropyi[response_maski > 0]
        k_boundary = max(int(high_entropy_ratio * valid_entropy.numel()), 1)

        # boundary = k_boundary-th largest value → top 20% are high-entropy
        boundary_vals, _ = torch.topk(valid_entropy, k_boundary)
        entropy_thres = boundary_vals[-1]

        low_mask_i  = (entropyi < entropy_thres) & (response_maski > 0)
        high_mask_i = (entropyi >= entropy_thres) & (response_maski > 0)
        low_masks.append(low_mask_i)
        high_masks.append(high_mask_i)

        # optional: clip mask for the very-lowest low-entropy tokens
        if clip_low_entropy_ratio is not None:
            n_low = low_mask_i.sum().item()
            k_clip = max(int(clip_low_entropy_ratio * n_low), 1)
            clip_vals, _ = torch.topk(entropyi[low_mask_i], k_clip, largest=False)
            clip_low_masks.append((entropyi < clip_vals[-1]) & low_mask_i)

        # optional: clip mask for the very-highest high-entropy tokens
        if clip_high_entropy_ratio is not None:
            n_high = high_mask_i.sum().item()
            k_clip = max(int(clip_high_entropy_ratio * n_high), 1)
            clip_vals, _ = torch.topk(entropyi[high_mask_i], k_clip)
            clip_high_masks.append((entropyi >= clip_vals[-1]) & high_mask_i)

    low_entropy_mask  = torch.stack(low_masks,  dim=0)
    high_entropy_mask = torch.stack(high_masks, dim=0)
    clip_low_entropy_mask  = torch.stack(clip_low_masks,  dim=0) if clip_low_masks  else None
    clip_high_entropy_mask = torch.stack(clip_high_masks, dim=0) if clip_high_masks else None
    return low_entropy_mask, high_entropy_mask, clip_low_entropy_mask, clip_high_entropy_mask


@register_policy_loss("htpo")  # type: ignore[arg-type]
def compute_policy_loss_htpo(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    entropy: torch.Tensor,
    advantages: torch.Tensor,
    ...
    """Compute the clipped policy objective and related metrics for HTPO.
    ...
    Args:
        ...
        entropy (torch.Tensor):
            Per-token entropies under the current policy, shape (batch_size, response_length).
        advantages (torch.Tensor):
            Advantage estimates, shape (batch_size, response_length).
        response_mask (torch.Tensor):
            Token mask (1 = valid, 0 = padding), shape (batch_size, response_length).
        difficulty (np.ndarray):
            difficulty flag per sample, shape (batch_size,).
        acc (np.ndarray):
            correctness flag per sample, shape (batch_size,).
        loss_agg_mode (str):
            Aggregation mode passed to ``agg_loss``. Defaults to "token-mean".
        config (ActorConfig):
            Actor configuration (must not be an AlgoConfig).
        rollout_is_weights (torch.Tensor | None):
            Optional importance-sampling weights from rollout correction.
    """
    assert config is not None
    assert not isinstance(config, AlgoConfig)
    clip_ratio = config.clip_ratio  # Clipping parameter ε for standard PPO. See https://arxiv.org/abs/1707.06347.
    ...
    pg_clip_num = 0
    pg_losses_list, inds_list = [], []

    # --- Group 1 & 2: hard prompt, correct answer ---
    correct_hard = np.logical_and(difficulty == True, acc == True)
    if correct_hard.any():
        clip_entropy_ratio1 = config.policy_loss.clip_entropy_ratio1 if config.policy_loss.clip_entropy_ratio1 is not None else 0.006
        assert clip_entropy_ratio1 > 0, "clip_entropy_ratio1 must be > 0."

        inds = np.nonzero(correct_hard)[0].tolist()
        inds_list.extend(inds)
        _advantages, _entropy, _response_mask, _log_prob, _ratio = (
            advantages[inds], entropy[inds], response_mask[inds], log_prob[inds], ratio[inds]
        )
        low_entropy_mask, high_entropy_mask, clip_low_entropy_mask, _ = _compute_entropy_split_masks(
            _entropy, _response_mask, clip_low_entropy_ratio=clip_entropy_ratio1
        )

        pg_losses_group12 = -_advantages * _ratio
        pg_losses_clip = -_advantages * torch.clamp(_ratio, 1 - cliprange_low, 1 + cliprange_high)
        
        if low_entropy_mask.sum() > 0:
            pg_clip_num += (pg_losses_clip[low_entropy_mask] > pg_losses_group12[low_entropy_mask]).sum()
            pg_losses_group12[low_entropy_mask] = torch.maximum(pg_losses_group12[low_entropy_mask], pg_losses_clip[low_entropy_mask]) * (~clip_low_entropy_mask[low_entropy_mask]).to(pg_losses_group12.dtype)
        
        upper_cliprange_mask = (_ratio > 1 + cliprange_high) & (high_entropy_mask)
        identity_ratio = torch.exp(_log_prob - _log_prob.detach())
        if upper_cliprange_mask.sum() > 0:
            pg_clip_num += upper_cliprange_mask.sum()
            pg_losses_group2_upper = (1 + cliprange_high) * identity_ratio * _advantages
            pg_losses_group12[upper_cliprange_mask] = pg_losses_group2_upper[upper_cliprange_mask]
        lower_cliprange_mask = (_ratio < 1 - cliprange_low) & (high_entropy_mask)
        if lower_cliprange_mask.sum() > 0:
            pg_clip_num += lower_cliprange_mask.sum()
            reverse_ratio = torch.exp(old_log_prob[inds] - _log_prob.detach())
            pg_losses_group2_lower = reverse_ratio * identity_ratio * _advantages
            pg_losses_group12[lower_cliprange_mask] = pg_losses_group2_lower[lower_cliprange_mask]
        
        pg_losses_list.append(pg_losses_group12)

    # --- Case 3 & 4: hard prompt, wrong answer ---
    wrong_hard = np.logical_and(difficulty == True, acc == False)
    if wrong_hard.any():
        inds = np.nonzero(wrong_hard)[0].tolist()
        inds_list.extend(inds)
        _advantages, _entropy, _response_mask, _log_prob, _ratio = (
            advantages[inds], entropy[inds], response_mask[inds], log_prob[inds], ratio[inds]
        )
        low_entropy_mask, high_entropy_mask, _, _ = _compute_entropy_split_masks(
            _entropy, _response_mask
        )

        pg_losses_group34 = -_advantages * _ratio
        pg_losses_clip = -_advantages * torch.clamp(_ratio, 1 - cliprange_low, 1 + cliprange_high)

        if low_entropy_mask.sum() > 0:
            pg_clip_num += (pg_losses_clip[low_entropy_mask] > pg_losses_group34[low_entropy_mask]).sum()
            pg_losses_group34[low_entropy_mask] = torch.maximum(pg_losses_group34[low_entropy_mask], pg_losses_clip[low_entropy_mask])

        lower_cliprange_mask = (_ratio < 1 - cliprange_low) & (high_entropy_mask)
        if lower_cliprange_mask.sum() > 0:
            pg_clip_num += lower_cliprange_mask.sum()
            identity_ratio = torch.exp(_log_prob - _log_prob.detach())
            pg_losses_group4_lower = (1 - cliprange_low) * identity_ratio * _advantages
            pg_losses_group34[lower_cliprange_mask] = pg_losses_group4_lower[lower_cliprange_mask]
        
        pg_losses_list.append(pg_losses_group34)

    # --- Case 5 & 6: easy prompt, correct answer ---
    correct_easy = np.logical_and(difficulty == False, acc == True)
    if correct_easy.any():
        clip_entropy_ratio2 = config.policy_loss.clip_entropy_ratio2 if config.policy_loss.clip_entropy_ratio2 is not None else 0.02
        assert clip_entropy_ratio2 > 0, "clip_entropy_ratio2 must be > 0."

        inds = np.nonzero(correct_easy)[0].tolist()
        inds_list.extend(inds)
        _advantages, _entropy, _response_mask, _log_prob, _ratio = (
            advantages[inds], entropy[inds], response_mask[inds], log_prob[inds], ratio[inds]
        )
        low_entropy_mask, high_entropy_mask, _, clip_high_entropy_mask = _compute_entropy_split_masks(
            _entropy, _response_mask, clip_high_entropy_ratio=clip_entropy_ratio2
        )

        pg_losses_group56 = -_advantages * _ratio
        pg_losses_clip = -_advantages * torch.clamp(_ratio, 1 - cliprange_low, 1 + cliprange_high)
        
        if low_entropy_mask.sum() > 0:
            pg_clip_num += (pg_losses_clip[low_entropy_mask] > pg_losses_group56[low_entropy_mask]).sum()
            pg_losses_group56[low_entropy_mask] = torch.maximum(pg_losses_group56[low_entropy_mask], pg_losses_clip[low_entropy_mask])

        if high_entropy_mask.sum() > 0:
            pg_clip_num += (pg_losses_clip[high_entropy_mask] > pg_losses_group56[high_entropy_mask]).sum()
            pg_losses_group56[high_entropy_mask] = torch.maximum(pg_losses_group56[high_entropy_mask], pg_losses_clip[high_entropy_mask]) * (~clip_high_entropy_mask[high_entropy_mask]).to(pg_losses_group56.dtype)

        pg_losses_list.append(pg_losses_group56)

    # --- Case 7 & 8: easy prompt, wrong answer ---
    wrong_easy = np.logical_and(difficulty == False, acc == False)
    if wrong_easy.any():
        inds = np.nonzero(wrong_easy)[0].tolist()
        inds_list.extend(inds)
        _advantages, _entropy, _response_mask, _log_prob, _ratio = (
            advantages[inds], entropy[inds], response_mask[inds], log_prob[inds], ratio[inds]
        )
        ...
        pg_losses_group78 = -_advantages * _ratio
        pg_losses_clip = -_advantages * torch.clamp(_ratio, 1 - cliprange_low, 1 + cliprange_high)

        if low_entropy_mask.sum() > 0:
            pg_clip_num += (pg_losses_clip[low_entropy_mask] > pg_losses_group78[low_entropy_mask]).sum()
            pg_losses_group78[low_entropy_mask] = torch.maximum(pg_losses_group78[low_entropy_mask], pg_losses_clip[low_entropy_mask])
        
        lower_cliprange_mask = (_ratio < 1 - cliprange_low) & (high_entropy_mask)
        if lower_cliprange_mask.sum() > 0:
            pg_clip_num += lower_cliprange_mask.sum()
            identity_ratio = torch.exp(_log_prob - _log_prob.detach())
            pg_losses_group8_lower = (1 - cliprange_low) * identity_ratio * _advantages
            pg_losses_group78[lower_cliprange_mask] = pg_losses_group8_lower[lower_cliprange_mask]
        
        pg_losses_list.append(pg_losses_group78)

    # Restore original sample order
    pg_losses = torch.cat(pg_losses_list)[np.argsort(inds_list).tolist()]

    pg_clipfrac = pg_clip_num / (response_mask > 0).sum()

    # Dual-clip: clamp losses when advantage < 0
    pg_losses2 = -advantages * clip_ratio_c
    dual_clip_pg_losses = torch.min(pg_losses2, pg_losses)
    pg_clipfrac_lower = verl_F.masked_mean(
        torch.gt(pg_losses, pg_losses2) * (advantages < 0).float(), response_mask
    )
    pg_losses = torch.where(advantages < 0, dual_clip_pg_losses, pg_losses)
    ...
    pg_metrics = {
        ...
        "actor/ppo_kl":            ppo_kl.detach().item(),
        "actor/pg_clipfrac_lower": pg_clipfrac_lower.detach().item(),
        "actor/micro_batch_size":  old_log_prob.shape[0],
    }
    return pg_loss, pg_metrics


@register_policy_loss("entropy")  # type: ignore[arg-type]
def compute_policy_loss_vanilla(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    entropy: torch.Tensor,
    ...
    values, _ = torch.topk(entropy[response_mask > 0], max(int(0.2 * response_mask.sum().item()), 1))  # 80% is low entropy tokens
    entropy_thres = values[-1]  
    high_entropy_mask = (entropy >= entropy_thres) & (response_mask > 0)
    ...
    if high_entropy_mask.sum() > 0:  # 只保留20% high entropy tokens
        pg_loss = agg_loss(
            loss_mat=pg_losses, loss_mask=response_mask * high_entropy_mask, loss_agg_mode=loss_agg_mode, **config.global_batch_info
        )
    else:
        pg_loss = agg_loss(
            loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode, **config.global_batch_info
...
def compute_policy_loss_bapo(
    ...
    # BAPO config
    adv_ratio_target = 0.5
    ratio_lower_start = 0.8
    ratio_lower_max = 0.95
    ratio_lower_step = 0.05
    ratio_upper_start = 1.2
    ratio_upper_max = 2.0
    ratio_upper_step = 0.1
    eps = 1.0e-8
    ...
    ratio_low_cur = ratio_lower_start if ratio_lower_start is not None else default_low
    ratio_high_cur = ratio_upper_start if ratio_upper_start is not None else default_high

    ratio_low_max = ratio_lower_max if ratio_lower_max is not None else ratio_low_cur
    ratio_high_max = ratio_upper_max if ratio_upper_max is not None else ratio_high_cur

    ratio_low_step = max(ratio_lower_step, 0.0)
    ratio_high_step = max(ratio_upper_step, 0.0)

    target_ratio = max(adv_ratio_target, 0.0)
